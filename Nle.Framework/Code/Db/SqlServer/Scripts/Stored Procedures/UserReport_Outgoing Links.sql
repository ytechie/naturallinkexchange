if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[UserReport_Outgoing Links]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[UserReport_Outgoing Links]
GO

CREATE PROCEDURE dbo.[UserReport_Outgoing Links]
	@UserId Int
AS

Exec Reporting_GetReportDefinition 
	@Type = 1,
	@Description = 'This reports shows a list of all outgoing links for each site that the user has.',
	@Title = ''

/* Report Section 1 Data */
Select 'This report is currently in development. Please note that this list may not be a ' +
				'perfect representation of the sites that you are linking to, due to numerous factors.' As [Information]
...
While (Select Count(*) From @Sites) > 0
	Begin
		...
		Where s.[Id] In (Select SiteId From dbo.[Links_GetOutgoingLinkedPartners](@CurrSiteId))
...
GRANT EXECUTE ON dbo.[UserReport_Outgoing Links] TO [Public]
Go
